          ...
          if (indexInDestConcat > -1) {
            destinationArray.forEach((word, wordIndex) => {
              if(word.indexOf(searchWord) > -1){
                ...
                penalty += indexScore*0.2*wordIndex;
                consoleOutput += `for  "${searchWord}" in "${word}" \n`;
                consoleOutput += `+${indexScore} = 200 / (${word.indexOf(searchWord)} + 1) \n`;
                consoleOutput += `-${penalty} = ${indexScore} * 0.2 * ${wordIndex} \n`;
              }
            })
            let startOfCityOrCountry = (dest.city.toLowerCase().indexOf(searchWord) == 0 || dest.country.toLowerCase().indexOf(searchWord) == 0) ? 100 : 0;
            indexScore += startOfCityOrCountry;
