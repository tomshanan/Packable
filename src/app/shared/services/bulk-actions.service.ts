...
export class BulkActionsService {
  ...
  public pushMissingCollectionsToProfiles(cGroups: tripCollectionGroup[]) {
    if (isDefined(cGroups)) {
      ...
      let importCollectionsIds: string[] = [];
      ...
      if (changes > 0) {
        if (importCollectionsIds.length > 0) {
          let importCollections = this.colFac.getCompleteByIdArray(importCollectionsIds)
  ...
  public processImportCollections(importCollections: CollectionComplete[], profileIds?: string[]): CollectionComplete[] {
    ...
    let selectedRemoteCollections = importCollections.filter(c => {
      return allRemoteCollections.hasId(c.id) && !localCollections.hasId(c.id)
    })
    let newOriginals = selectedRemoteCollections.map(c => this.colFac.completeToOriginal(c))
    ...
    let selectedLocal = localCollections.filter(c => importCollections.hasId(c.id))
    let allComplete = [...selectedLocal, ...selectedRemoteCollections]
  ...
  public addPackablesFromRemoteCollections(collections: CollectionComplete[]) {
   ...
   */
  public pushCollectionSettingsToProfiles(collection: CollectionComplete, profileIds: string[]) {
    if (collection && profileIds.length > 0) {
      let privateCollection = this.colFac.completeToPrivate(collection)
      let profiles = this.storeSelector.profiles.filter(p => profileIds.includes(p.id));
      privateCollection.dateModified = timeStamp()
      profileIds.forEach(pId => {
        let profile = profiles.findId(pId)
        profile.dateModified = timeStamp()
        let index = profile.collections.idIndex(privateCollection.id)
        if (index !== -1) {
          profile.collections[index] = {
            ...privateCollection,
            packables: profile.collections[index].packables
          }
        }
      })

      console.log(`pushCollectionSettingsToProfiles`, profiles)
      this.store.dispatch(new profileActions.editProfiles(profiles))
    }
  }
  ...
  public pushOriginalPackablesByCP(packables: PackableOriginal[], CPs: CollectionProfile[]) {
    if (CPs && CPs.length > 0) {
      let privatePackables = packables.map(packable => {
        packable.dateModified = timeStamp();
        return this.pacFac.makePrivate(packable)
      })
      this.pushPrivatePackablesByCP(privatePackables, CPs)
    }
  }
  private pushPrivatePackablesByCP(privatePackables: PackablePrivate[], CPs: CollectionProfile[]) {
    const debuggerArray = []
    debuggerArray.push('pushPrivatePackablesByCP\nreceived private packables:',privatePackables,'\nand received CPs:',CPs,'\n')
    if (CPs && CPs.length > 0 && privatePackables && privatePackables.length > 0) {
      ...
      debuggerArray.push('Modified Profiles:',modifiedProfiles)
      if (modifiedProfiles.length > 0) {
        this.store.dispatch(new profileActions.editProfiles(modifiedProfiles))
      }
      debuggerArray.push('Modified Collections:',modifiedCollections)
      if (modifiedCollections.length > 0) {
        this.store.dispatch(new collectionActions.updateOriginalCollections(modifiedCollections))
      }
    } else { debuggerArray.push('The variables were not valid')}
    console.log(...debuggerArray)
  }
}
